...
class PostTableViewCell: UITableViewCell {
    ...
    private lazy var deleteIcon: UIImageView = {
        let imageView = UIImageView()
        imageView.image = UIImage(named: "delete_icon")?.withTintColor(.white)
        imageView.contentMode = .scaleAspectFit
        return imageView
    } ()
    ...
    private func setupClicks() {
        ...
        deleteIcon.addGestureRecognizer(deleteGestureRecognizer)
    }
    
    private func setupDeleteButton()  {
        if Singleton.shared.user?.isAdmin == true  {
            self.contentView.addSubview(deleteIcon)
            deleteIcon.translatesAutoresizingMaskIntoConstraints = false
            NSLayoutConstraint(item: deleteIcon, attribute: .top, relatedBy: .equal, toItem: collectionView, attribute: .bottom, multiplier: 1, constant: 10).isActive = true
            NSLayoutConstraint(item: deleteIcon, attribute: .trailing, relatedBy: .equal, toItem: self.contentView, attribute: .trailing, multiplier: 1, constant: -20).isActive = true
            NSLayoutConstraint(item: deleteIcon, attribute: .height, relatedBy: .equal, toItem: .none, attribute: .notAnAttribute, multiplier: 1, constant: 20).isActive = true
            NSLayoutConstraint(item: deleteIcon, attribute: .width, relatedBy: .equal, toItem: deleteIcon, attribute: .height, multiplier: 1, constant: 0).isActive = true
        }
    }
}
