...
extension AlamofireService: APIContract {
    ...
    func deletePosts(postId: String, callback: APICallback<[BaseProp]>) {
        endpointCall(path: VipsAPIEndpoints.DELETE_POSTS + "?post_id=\(postId)", params: noneParam) { (risks: [BaseProp]) -> Void in
            callback.success(risks)
        } failure: { error in
            callback.failure(error)
        }
